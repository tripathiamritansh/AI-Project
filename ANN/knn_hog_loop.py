...
for i in range(10):
	...
	clf_knn_hog.fit(hogFeatures_train, train_y)
	y_pred_knn = clf_knn_hog.predict(hogFeatures_test)
	acc_knn_hog = accuracy_score(test_y, y_pred_knn)
	arr_hog_acc.append(acc_knn_hog)
...
blue_patch = mpatches.Patch(color='blue', label='KNN w/o hog')
red_patch = mpatches.Patch(color='red', label='KNN w/ hog')
plt.legend(handles=[blue_patch, red_patch], loc=4)
plt.savefig('knn_hog.png')
plt.show()
